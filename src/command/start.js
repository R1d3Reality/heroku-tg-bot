...
function sendSimpleUserResponse(userId, bot) {
    return bot.sendMessage(userId, 'Hello. I can help you to get AWESOME PHOTO. Write /help to know more.');
}

function sendRepairerUserResponse(userId, bot) {
    return bot.sendMessage(userId, 'You have been registered as a photoHERO. You will get requests as soon as new are submitted.');
}
        ...
        return this.userDao.findRepairerByStartId(startId)
            .then(user => {
                if (user) {
                    ...
                    log.verbose('Start', `Found photographer user, will set userId for him`);
